...
class AELoss(nn.Module):
    def __init__(self, loss_type, max_num_people, output_resolution):
        super().__init__()
    ...
    def batchTagLoss(self, pred_tag_map, joints):
        '''
        pred_tag_map: (B, K * H * W)
        joints: (B, 30, K, 2)
        '''
        ...
        joints_vis = joints[:, :, :, 1].float()  # (B, 30, K)
        person_joints_cnt = joints_vis.sum(2, keepdim=True)  # (B, 30, 1)
        joints_loc = joints[:, :, :, 0].reshape(batch_size, -1).long()  # (B, 30 * K)

        tags = torch.gather(pred_tag_map, index=joints_loc, dim=1)  # (B, 30 * K)
        tags = tags.reshape(batch_size, max_person, num_joints) * joints_vis  # (B, 30, K)

        person_cnt = (person_joints_cnt > 0).float().squeeze(2).sum(dim=1, keepdim=True)  # (B, 1)
        person_cnt[person_cnt == 0] = 1
        person_vis = (person_joints_cnt > 0).expand(batch_size, max_person, max_person).float()  # (B, 30, 30)
        person_vis = person_vis * person_vis.permute(0, 2, 1)
        person_joints_cnt[person_joints_cnt == 0] = 1

        # PULL
        tags_mean = tags.sum(2, keepdim=True) / person_joints_cnt  # (B, 30, 1)
        # tags_mean[person_joints_cnt == 0] = 0  # clear nan elements from divided by zero
        assert torch.isnan(tags_mean).sum() == 0

        pull = torch.sum(joints_vis * (tags - tags_mean) ** 2, dim=2, keepdim=True) / person_joints_cnt  # (B, 30, 1)
        pull[person_joints_cnt == 0] = 0

        pull = pull.squeeze(2).sum(1, keepdim=True) / person_cnt
        # pull[person_cnt == 0] = 0  # clear nan elements from divided by zero
        pull = torch.mean(pull)

        # PUSH
        tags_mean = (tags_mean).expand(batch_size, max_person, max_person)  # (B, 30, 30)
        diff = (tags_mean - tags_mean.permute(0, 2, 1)) * person_vis  # (B, 30, 30)

        if self.loss_type == 'exp':
            diff = torch.exp(- diff ** 2) * person_vis  # (B, 30, 30)
            diff = 0.5 * (torch.sum(diff, dim=(1, 2)) - person_cnt.squeeze(1)) / torch.clamp((person_cnt - 1) * person_cnt, min=1).squeeze(1)  # (B)
            diff[person_cnt.squeeze(1) < 2] = 0

            push = torch.mean(diff)

        elif self.loss_type == 'max':
            diff = 1 - torch.abs(diff)  # (B, 30, 30)
            diff = torch.clamp(diff, min=0) * person_vis
            diff = 0.5 * (torch.sum(diff, dim=(1, 2)) - person_cnt.squeeze(1)) / torch.clamp((person_cnt - 1) * person_cnt, min=1).squeeze(1)  # (B)
            diff[person_cnt.squeeze(1) < 2] = 0
    ...
    def forward(self, tags, joints):
        ...
        joints: (B, 30, K, 2)
        """
        batch_push, batch_pull = self.batchTagLoss(tags, joints)

        return batch_push, batch_pull
...
class LossFactory(nn.Module):
    def __init__(self, cfg):
        ...
        if cfg.LOSS.WITH_AE_LOSS:
            self.ae_loss = AELoss(cfg.LOSS.AE_LOSS_TYPE, cfg.DATASET.MAX_NUM_PEOPLE, cfg.DATASET.OUTPUT_SIZE)
...
class MultiLossFactory(nn.Module):
    def __init__(self, cfg):
        ...
        self.ae_loss = \
            nn.ModuleList(
                [
                    AELoss(cfg.LOSS.AE_LOSS_TYPE, cfg.DATASET.MAX_NUM_PEOPLE, cfg.DATASET.OUTPUT_SIZE[index]) if cfg.LOSS.WITH_AE_LOSS[index] else None
                    for index in range(len(cfg.LOSS.WITH_AE_LOSS))
                ]
            )
        self.push_loss_factor = cfg.LOSS.PUSH_LOSS_FACTOR
        self.pull_loss_factor = cfg.LOSS.PULL_LOSS_FACTOR
